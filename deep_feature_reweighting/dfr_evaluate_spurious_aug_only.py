...
parser.add_argument(
    "--augset_dir", type=str,
    default=None,
    help="Augmentation set data directory path.")
parser.add_argument(
...
def dfr_train_subset_tune(
    ...
    n_groups = np.max(g_train) + 1
    # g_idx = [np.where(g_train == g)[0] for g in range(n_groups-1)]
    ...
    g_idx = [np.where(g_train == g)[0] for g in [0,3]]
    for g in g_idx:
        np.random.shuffle(g)
    ...
    x_train = np.concatenate([x_train[g[:min_g]] for g in g_idx] + [x_aug])
    y_train = np.concatenate([y_train[g[:min_g]] for g in g_idx] + [y_aug])
    g_train = np.concatenate([g_train[g[:min_g]] for g in g_idx] + [g_aug])

    print(f"Y train count: {np.array(np.unique(y_train, return_counts=True)).T}")
    print(np.bincount(g_train))
    if preprocess:
        x_train = scaler.transform(x_train)
        x_val = scaler.transform(x_val)
...
def dfr_train_subset_eval(
    ...
    for i in range(num_retrains):
        ...
        # for idx, g in enumerate(g_idx):
        #     if len(g) == min_g:
        #         min_g_idx = idx

        # print(f"min_g_idx : {min_g_idx}")

        # g_idx = [np.where(g_train == g)[0] for g in [0,1,2]]
        ...
        # print()
        # print(f"g_train before : {g_train}")
        # print()
        # print(f"g_aug before : {g_aug}")

        # g_aug_idx = [np.where(g_aug == g)[0] for g in range(n_groups)]
        ...
        x_train = np.concatenate([x_train[g[:min_g]] if idx!=3 else x_train[g[:10]] for idx, g in enumerate(g_idx)] + [x_aug])
        y_train = np.concatenate([y_train[g[:min_g]] if idx!=3 else y_train[g[:10]] for idx, g in enumerate(g_idx)] + [y_aug])
        g_train = np.concatenate([g_train[g[:min_g]] if idx!=3 else g_train[g[:10]] for idx, g in enumerate(g_idx)] + [g_aug])
        # print()
        # print(f"g_train after : {g_train}")
        # print()
        print(np.bincount(g_train))
        # exit()
        if preprocess:
...
augset = WaterBirdsDataset(
    basedir=args.augset_dir, split="train", transform=train_transform)
...
val_loader = get_loader(
    ...
    **loader_kwargs)
aug_loader = get_loader(
    augset, train=True, reweight_groups=False, reweight_classes=False,
    **loader_kwargs)
# Load model
...
all_embeddings = {}
all_y, all_p, all_g = {}, {}, {}
for name, loader in [("train", train_loader), ("test", test_loader), ("val", val_loader), ("aug", aug_loader)]:
    all_embeddings[name] = []
