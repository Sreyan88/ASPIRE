...
from hard_imagenet_data import HardImageNetDataset, get_loader, get_transform_cub, log_data

from utils_hi import MultiTaskHead
from utils_hi import Logger, AverageMeter, set_seed, evaluate, get_y_p
from utils_hi import update_dict, get_results, write_dict_to_tb
...
trainset = HardImageNetDataset(basedir=basedir, split="train", transform=train_transform)
testset_dict = {
    'wb': HardImageNetDataset(basedir=args.test_wb_dir, split="test", transform=test_transform),
    'wb_val': HardImageNetDataset(basedir=args.test_wb_dir, split="val", transform=test_transform),
}

if not args.predict_place and not (args.test_grey_dir is None):
    testset_dict['grey'] = HardImageNetDataset(basedir=args.test_grey_dir, split="test", transform=test_transform)
if ((args.predict_place) and not (args.test_places_dir is None)) or args.multitask:
    testset_dict['places'] = HardImageNetDataset(basedir=args.test_places_dir, split="test", transform=test_transform)
...
# print()
# print()
# print(trainset.metadata_df)

# trainset.metadata_df.to_csv("cel_spu_only.csv", index=False)

# exit()
testset = HardImageNetDataset(basedir=basedir, split="test", transform=test_transform)
valset = HardImageNetDataset(basedir=basedir, split="val", transform=test_transform)
...
# model = torchvision.models.vit_l_32(pretrained=args.pretrained_model)
...
for epoch in range(args.num_epochs):
    ...
    if args.multitask:
        results_place = get_results(acc_place_groups)
    ...
    if epoch % args.eval_freq == 0:
        ...
        for test_name, test_loader in test_loader_dict.items():
            results = evaluate(model, test_loader, args.multitask, args.predict_place)
