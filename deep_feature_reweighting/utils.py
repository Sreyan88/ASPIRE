...
def transform_element(element):
    return labels_dict.get(element)

def update_dict(acc_groups, y, g, logits):
    preds = torch.argmax(logits, axis=1)
    # y_act = y.clone()
    # for i in range(y.size(0)):
    #     y_act[i] = transform_element(y[i])
    # y_act = y.map(transform_element)(y)
...
def evaluate(model, loader, get_yp_func, multitask=False, predict_place=False):
    model.eval()
    print(range(loader.dataset.n_groups))
    acc_groups = {g_idx : AverageMeter() for g_idx in range(loader.dataset.n_groups)} #[13392, 6656, 9632, 6064, 15696, 12816, 12960, 8592, 14240, 6928, 11296, 12720, 12560, 10480, 11936]}#range(loader.dataset.n_groups)}
    if multitask:
